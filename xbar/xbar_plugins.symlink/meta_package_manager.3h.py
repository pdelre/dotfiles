...
def print_menu():
    ...
    # Ignore some packages - Docker pre-relicense
    for manager in managers:
        if manager['name'] == 'Homebrew Cask':
            for package in manager['packages']:
                if package['name'] == 'docker':
                    manager['packages'].remove(package)

            if 'upgrade_all_cli' in manager.keys() and len(manager['packages']) == 0:
                manager.pop('upgrade_all_cli')
    ...
    if total_outdated > 0:
        ...
        echo("🆗{} | dropdown=false | size=14".format(
            " ⚠️{}".format(total_errors) if total_errors else ""))
